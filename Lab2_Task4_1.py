...
def checked_access(L:list[int], idx:int) -> Optional[int]:
    test = idx >= 0 and idx < len(L)    # What is the value of test on each call? False, True
    if test:                            # What is this check preventing? Prevents returning a value greater than the length of the list
        return L[idx]
    else:
        return None
